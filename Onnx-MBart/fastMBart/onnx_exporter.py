...
from .onnx_models_structure import (
    MBartEncoder,
    MBartDecoderWithLMhead,
    MBartDecoderWithLMheadInitial,
)
...
_folder = Path.cwd()
saved_models_path = _folder.joinpath("models-mbart")

Bar.check_tty = False


def create_bart_encoder_decoder(pretrained_version="facebook/mbart-large-en-ro"):
    ...
    model = MBartForConditionalGeneration.from_pretrained(pretrained_version, use_auth_token=get_auth_token())
...
def turn_model_into_encoder_decoder(model):
    ...
    decoder_with_lm_head = MBartDecoderWithLMhead(decoder, lm_head, final_logits_bias, model.config)
    simplified_encoder = MBartEncoder(encoder)
    decoder_with_lm_head_init = MBartDecoderWithLMheadInitial(decoder, lm_head, final_logits_bias, model.config)
...
def generate_onnx_representation(
    ...
    model_config = MBartConfig.from_pretrained(pretrained_version, use_auth_token=get_auth_token())
...
def get_model_paths(pretrained_model, model_path, quantized):
    ...
    print(f"init_decoder_path : {init_decoder_path}")
    return encoder_path, decoder_path, init_decoder_path
