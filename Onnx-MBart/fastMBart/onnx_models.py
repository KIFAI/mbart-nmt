...
class MBartEncoder(torch.nn.Module):
...
class OnnxMBart(MBartForConditionalGeneration):
    ...
    def __init__(self, model_or_model_path, onnx_model_sessions):
        ...
        self.encoder = MBartEncoder(encoder_sess)
        self.decoder = MBartDecoder(decoder_sess)
        self.decoder_init = MBartDecoderInit(decoder_sess_init)
...
def export_and_get_onnx_model(
    model_or_model_path, custom_output_path=saved_models_path, quantized=False
):
    """
...
def get_onnx_model(
    model_name_or_path, onnx_models_path=saved_models_path, quantized=False
):
    """
    ...
    model = OnnxMBart(model_name_or_path, model_sessions)
